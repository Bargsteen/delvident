...
import Hyper.Node.Server (defaultOptionsWithLogging, runServer', Port(..))
import Hyper.Response (closeHeaders, contentType, respond, writeStatus)
import Hyper.Trout.Router (RoutingError(..), router)
import Hyper.Status (statusNotFound)
import API (Task(..), TaskId, api)

type AppM a
  = ExceptT RoutingError (ReaderT (Array Task) Aff) a
...
main = do
  log "Running server!!"
  runServer' (defaultOptionsWithLogging { port = Port 1234 }) {} (flip runReaderT tasks) siteRouter
  where
  ...
  siteRouter = router api resources onRoutingError
